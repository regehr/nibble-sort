...
void nibble_sort_beekman2(uint64_t *buf) {
  ...
  // shuffle 8 can be done in place
  __m256i shuf8 = {0x1715141613121110ULL, 0x1f1e1c1b1a19181dULL,
                   0x0705040603020100ULL, 0x0f0e0c0b0a09080dULL};
  __m256i shuf8a = {0x1716141312111015ULL, 0x1f1e1c1b1a19181dULL,
                    0x0706040302010005ULL, 0x0f0e0c0b0a09080dULL};
  __m256i shuf8b = {0x1715141613121110ULL, 0x1f1d1c1e1b1a1918ULL,
                    0x0705040603020100ULL, 0x0f0d0c0e0b0a0908ULL};
  ...
  for (uint32_t i = 0; i < (1024 / 4); i += 1) {
    __m256i r0 = _mm256_load_si256(((__m256i *)buf) + i), r1 = r0, r2;
    r0 &= nibblemask;
    r1 ^= r0;
...
#define sort_and_shuffle(n)                                                    \
  ...
  r1 = (__m256i)_mm256_shuffle_pd((__m256d)r1, (__m256d)r2, 0b1111)
    ...
    { // sort_and_shuffle(8);
      r2 = _mm256_max_epi8(r0, r1);
      r0 = _mm256_min_epi8(r0, r1);
      r1 = _mm256_shuffle_epi8(r0, shuf8b);
      r0 = _mm256_shuffle_epi8(r2, shuf8a);
    }
    sort_and_shuffle(9);

    r1 = _mm256_slli_epi64(r1, 4);
    _mm256_store_si256(((__m256i *)buf) + i, r1 | r0);
  }
}
